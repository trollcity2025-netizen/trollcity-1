-- Complete Staff Meeting System Setup
-- Run this SQL to create all tables, functions, and policies
...
CREATE OR REPLACE FUNCTION can_start_staff_meeting(p_user_id UUID)
RETURNS BOOLEAN AS $$
DECLARE
  profile RECORD;
BEGIN
...
CREATE OR REPLACE FUNCTION can_schedule_staff_meeting(p_user_id UUID)
RETURNS BOOLEAN AS $$
DECLARE
  profile RECORD;
BEGIN
...
CREATE OR REPLACE FUNCTION get_staff_meeting_permission(p_user_id UUID)
...
CREATE OR REPLACE FUNCTION is_staff_meeting_full(p_meeting_id UUID)
RETURNS BOOLEAN AS $$
DECLARE
...
BEGIN
  ...
  WHERE p.meeting_id = p_meeting_id AND p.is_active = TRUE
...
DROP POLICY IF EXISTS "Staff can view staff meetings" ON staff_meetings;
DROP POLICY IF EXISTS "Authorized users can insert staff meetings" ON staff_meetings;
DROP POLICY IF EXISTS "Starters can update staff meetings" ON staff_meetings;
...
DROP POLICY IF EXISTS "Staff can view participants" ON staff_meeting_participants;
DROP POLICY IF EXISTS "Participants can insert themselves" ON staff_meeting_participants;
DROP POLICY IF EXISTS "Participants can update their own status" ON staff_meeting_participants;
...
CREATE POLICY "Participants can insert themselves" ON staff_meeting_participants
  FOR INSERT WITH CHECK (
    staff_meeting_participants.user_id = auth.uid() AND 
    can_access_staff_meeting(auth.uid())
  );
