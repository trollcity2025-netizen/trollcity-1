...
export default function TrollopolyCityDevTest() {
  ...
  const createCityBoard = (scene: THREE.Scene) => {
    ...
    // Trees in outer grass only
    const roadEdge = size / 2 - streetWidth / 2;
    const treeOffset = roadEdge + 5;
    const treePositions = [
      { x: -30, z: -30 }, { x: -20, z: -30 }, { x: 20, z: -30 }, { x: 30, z: -30 },
      { x: -30, z: 30 }, { x: -20, z: 30 }, { x: 20, z: 30 }, { x: 30, z: 30 },
      { x: -35, z: -20 }, { x: -35, z: 0 }, { x: -35, z: 20 },
      { x: 35, z: -20 }, { x: 35, z: 0 }, { x: 35, z: 20 },
      { x: -20, z: -treeOffset }, { x: 0, z: -treeOffset }, { x: 20, z: -treeOffset },
      { x: -20, z: treeOffset }, { x: 0, z: treeOffset }, { x: 20, z: treeOffset }
    ];
    treePositions.forEach((pos) => {
      const treeGroup = new THREE.Group();
      const scale = 1 + Math.random() * 0.3;
      const trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.3 * scale, 0.4 * scale, 1.8 * scale), new THREE.MeshStandardMaterial({ color: 0x8b4513 }));
      trunk.position.y = 0.9 * scale;
      trunk.castShadow = true;
      treeGroup.add(trunk);
      for (let i = 0; i < 3; i++) {
        const leaves = new THREE.Mesh(new THREE.ConeGeometry((1.6 - i * 0.35) * scale, 2.2 * scale, 8), new THREE.MeshStandardMaterial({ color: 0x228b22 }));
        leaves.position.y = (2.2 + i * 1.3) * scale;
        leaves.castShadow = true;
        treeGroup.add(leaves);
      }
      treeGroup.position.set(pos.x + (Math.random() - 0.5) * 2, 0, pos.z + (Math.random() - 0.5) * 2);
  ...
  const createDice = (scene: THREE.Scene) => {
    ...
    const dieGeometry = new THREE.BoxGeometry(1.2, 1.2, 1.2);
    const dieMaterial = new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 0.4 });

    for (let i = 0; i < 2; i++) {
      const die = new THREE.Mesh(dieGeometry, dieMaterial);
      die.position.set(i === 0 ? -2 : 2, 12 + i, i === 0 ? -2 : 2);
  ...
  const rollDice = () => {
    ...
    setTimeout(() => {
      ...
      diceRef.current.forEach((die, i) => {
        ...
        die.rotation.x = rotations[value].x;
        die.rotation.y = rotations[value].y;
        die.position.y = 0.6;
      });
    }, 2500);
