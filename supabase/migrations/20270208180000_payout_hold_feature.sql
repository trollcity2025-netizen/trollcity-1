-- Migration: Add Payout Hold/Release Configuration
-- 1. Insert default setting (Disabled for Week 1)
INSERT INTO admin_app_settings (setting_key, setting_value, description)
VALUES (
  'payout_config', 
  '{"enabled": false, "message": "Payouts are currently on hold for Week 1."}', 
  'Global configuration for automated payouts'
)
ON CONFLICT (setting_key) DO UPDATE
SET setting_value = EXCLUDED.setting_value;

-- 2. Update prepare_payout_run to respect this setting
...
BEGIN
  ...
  WHERE setting_key = 'payout_config';

  v_enabled := COALESCE((v_config->>'enabled')::boolean, false);

  IF NOT v_enabled THEN
    -- Optionally log this attempt or just exit?
    -- We'll raise a notice and return NULL so the edge function knows to stop.
    RAISE NOTICE 'Payouts are currently disabled.';
    RETURN NULL;
  END IF;
