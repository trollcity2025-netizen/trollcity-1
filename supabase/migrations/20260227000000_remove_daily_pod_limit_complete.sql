-- Migration: Remove Daily Pod Limit Completely
-- Description: Removes the daily pod limit from can_start_pod function.
--              Only the 1 simultaneous pod limit remains for LMPM.
...
BEGIN
    ...
    IF COALESCE(v_lmpm_enabled, false) AND v_is_before_deadline THEN
        -- Rule: 1 simultaneous pod globally (daily limit REMOVED)
        ...
        -- Daily pod limit has been removed - users can now create unlimited pods per day
    END IF;
