...
BEGIN
  ...
  SELECT ARRAY_AGG(q.* ORDER BY q.order_index, q.id) INTO v_questions
  ...
  v_total_questions := array_length(v_questions, 1);

  -- Grade answers
  FOREACH v_question IN ARRAY v_questions
  LOOP
...
BEGIN
  ...
  -- Get recent attempts
  SELECT ARRAY_AGG(a.* ORDER BY a.completed_at DESC) INTO v_attempts
...
COMMENT ON FUNCTION assign_officer_orientation IS 'Assigns orientation to a user after application approval';
COMMENT ON FUNCTION start_officer_orientation IS 'Starts an orientation session';
COMMENT ON FUNCTION submit_officer_quiz IS 'Submits quiz answers and grades them';
COMMENT ON FUNCTION get_officer_orientation_status IS 'Gets the current orientation status for a user';
