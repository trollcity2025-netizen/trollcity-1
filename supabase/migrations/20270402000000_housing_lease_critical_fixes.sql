...
BEGIN
  -- Get lease
  SELECT * INTO v_lease FROM public.leases WHERE id = p_lease_id AND tenant_id = v_user_id AND status = 'active';
  IF NOT FOUND THEN
    RETURN jsonb_build_object('success', false, 'error', 'Active lease not found');
  END IF;
...
BEGIN
  -- Get lease
  SELECT * INTO v_lease FROM public.leases WHERE id = p_lease_id AND status = 'active';
  IF NOT FOUND THEN
    RETURN jsonb_build_object('success', false, 'error', 'Active lease not found');
  END IF;
