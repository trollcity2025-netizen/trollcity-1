// create-square-customer Edge Function
// Creates a Square customer for a user and saves card on file
...
Deno.serve(async (req) => {
  const requestId = `sq_cust_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
  
  console.log(`[CreateSquareCustomer ${requestId}] Request received`)
  ...
  try {
    ...
    const { userId, customerId: existingCustomerId, cardNonce } = body
    ...
    // Get user profile
    const profileRes = await fetch(`${supabaseUrl}/rest/v1/user_profiles?id=eq.${userId}&select=id,username,email,square_customer_id,square_card_id`, {
    ...
    if (!customerId) {
      console.log(`[CreateSquareCustomer ${requestId}] Creating new Square customer`)
      ...
      if (!customerRes.ok) {
        ...
        console.error(`[CreateSquareCustomer ${requestId}] Customer creation failed:`, err)
    ...
    // If card nonce provided, create card
    let cardId = profile?.square_card_id
    if (cardNonce && customerId) {
      console.log(`[CreateSquareCustomer ${requestId}] Adding card to customer`)
      ...
      if (!cardRes.ok) {
        ...
        console.error(`[CreateSquareCustomer ${requestId}] Card creation failed:`, err)
      ...
      if (cardId) {
        ...
        if (cardId) {
          ...
          console.log(`[CreateSquareCustomer ${requestId}] Card saved: ${cardId}`)
        }
      } else {
        const err = await cardRes.json()
        console.error(`[CreateSquareCustomer ${requestId}] Card creation failed:`, err)
    ...
    console.error(`[CreateSquareCustomer ${requestId}] Error:`, err)
