...
-- Create troll_wheel_spins table for tracking wheel usage
CREATE TABLE IF NOT EXISTS troll_wheel_spins (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_troll_wheel_spins_user_id ON troll_wheel_spins(user_id);
CREATE INDEX IF NOT EXISTS idx_troll_wheel_spins_created_at ON troll_wheel_spins(created_at);

-- Enable RLS
ALTER TABLE trollz_transactions ENABLE ROW LEVEL SECURITY;
ALTER TABLE bonus_coin_transactions ENABLE ROW LEVEL SECURITY;
ALTER TABLE troll_wheel_spins ENABLE ROW LEVEL SECURITY;
...
ON troll_wheel_spins FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own wheel spins" 
ON troll_wheel_spins FOR INSERT 
WITH CHECK (auth.uid() = user_id);
