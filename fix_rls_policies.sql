...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'admins') 
       AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tcnn_editors') THEN
    ...
    ELSIF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'admins') THEN
        EXECUTE 'DROP POLICY IF EXISTS "Allow all inserts" ON public.tcnn_articles';
        EXECUTE 'CREATE POLICY "Allow all inserts" ON public.tcnn_articles
            FOR INSERT WITH CHECK (
                current_setting(''app.service_role_key'', true) IS NOT NULL
