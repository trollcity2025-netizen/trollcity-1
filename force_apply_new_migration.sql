...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM training_scenarios LIMIT 1) THEN
    INSERT INTO training_scenarios (scenario_name, scenario_type, event_type, description, chat_messages, correct_action, points_awarded, difficulty_level)
    VALUES
      ('harassment_scenario', 'harassment', 'harassment', 'User repeatedly insults another viewer',
       ...
       'warn', 10, 1),
      ('spam_scenario', 'spam', 'spam', 'User sends same message 10 times in a row',
       ...
       'mute', 10, 1),
      ('scam_scenario', 'scam', 'scam', 'User trying to sell fake coins',
       ...
       'ban', 20, 2),
      ('self_harm_scenario', 'self_harm', 'self_harm', 'User mentions self-harm',
