...
export default function OfficerTrainingSimulator() {
  ...
  const loadScenario = async () => {
    ...
    try {
      // Try to fetch existing scenarios from DB
      const { data: scenarios, error } = await supabase
        .from('training_scenarios')
        .select('*')
      
      if (error) {
        console.warn('Error fetching scenarios, using fallback:', error)
      }

      let activeList = scenarios || []
      
      if (activeList.length === 0) {
        console.log('No scenarios in DB, using fallback data')
        activeList = FALLBACK_SCENARIOS
      }

      // Pick random from existing
      const activeScenario = activeList[Math.floor(Math.random() * activeList.length)] as TrainingScenario
      setScenario(activeScenario)
    } catch (error: any) {
      console.error('Error loading scenario:', error)
      toast.error(error.message || 'Failed to load training scenario')
      setScenario(null)
    } finally {
      setLoading(false)
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isCorrect) {
        ...
        toast.error(`Incorrect. Correct action: ${scenario.correct_action}`)
      }
    } catch (error: any) {
