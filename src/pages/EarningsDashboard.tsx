      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {loading ? (
          ...
          <>
            ...
            {(summary?.pending_requests_count ?? 0) > 0 && (
              <div className="bg-blue-900/30 border border-blue-500/50 rounded-xl p-4">
                <div className="flex items-center gap-3">
                  <Clock className="w-5 h-5 text-blue-400" />
                  <div>
                    <p className="font-semibold text-blue-400">
                      {(summary?.pending_requests_count ?? 0)} Pending Payout Request{(summary?.pending_requests_count ?? 0) > 1 ? 's' : ''}
                    </p>
                    <p className="text-sm text-gray-300">
                      Your payout request{(summary?.pending_requests_count ?? 0) > 1 ? 's are' : ' is'} being reviewed by our team.
                    </p>
                  </div>
                </div>
              </div>
