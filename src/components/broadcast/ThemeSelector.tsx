...
export default function ThemeSelector({ streamId, currentThemeUrl, onClose }: ThemeSelectorProps) {
    ...
    const fetchThemes = useCallback(async () => {
        ...
        try {
            ...
            // Map the data to match the ThemeItem interface if needed
            const mappedThemes = (data || []).map(t => ({
                id: t.id,
                name: t.name,
                preview_url: t.preview_url || t.image_url,
                is_premium: t.price_coins > 0,
                cost_coins: t.price_coins
            }));
    ...
    const handleSelectTheme = async (theme: ThemeItem | null) => {
        if (!user) return;
        const themeUrl = theme?.preview_url || null;
        ...
        try {
            ...
            if (error) throw error;
            toast.success(themeUrl ? `Theme "${theme?.name}" activated` : "Theme reset");
        } catch (e) {
            console.error(e);
        ...
        <div className="absolute bottom-full right-0 w-80 mb-4 bg-zinc-900 border border-purple-500/30 rounded-xl p-4 shadow-2xl z-[100] animate-in slide-in-from-bottom-2">
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-2 gap-2 max-h-80 overflow-y-auto pr-1">
                    ...
                    {themes.map((theme) => {
                        const isActive = currentThemeUrl === theme.preview_url;
                        return (
                            <button
                                key={theme.id}
