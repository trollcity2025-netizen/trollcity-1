-- Fix accept_battle to initialize box_count to 1 for both streams when battle starts
-- This ensures box count doesn't reset on page reload

-- Drop existing function first
DROP FUNCTION IF EXISTS public.accept_battle(UUID);

-- Create new function with box_count initialization
CREATE FUNCTION public.accept_battle(p_battle_id UUID)
RETURNS BOOLEAN AS $$
DECLARE
    v_battle RECORD;
...
BEGIN
    ...
    -- 5. Set battle_id on both streams and initialize box_count to 1
    UPDATE public.streams SET battle_id = p_battle_id, is_battle = true, box_count = COALESCE(box_count, 1) WHERE id = v_battle.challenger_stream_id;
    UPDATE public.streams SET battle_id = p_battle_id, is_battle = true, box_count = COALESCE(box_count, 1) WHERE id = v_battle.opponent_stream_id;
