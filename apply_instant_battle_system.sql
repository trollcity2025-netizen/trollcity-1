...
-- ============================================================================
-- This migration implements automatic battle matching based on click timing.
-- When both broadcasters click "Find Match", they are automatically connected.
-- 
-- New Flow:
-- 1. Broadcaster clicks "Find Match" → added to battle_queue
-- 2. If another broadcaster is waiting → battle starts automatically
-- 3. No acceptance required - battle begins instantly when two people queue
-- ============================================================================

-- ============================================================================
-- STEP 1: Create Simple Matchmaking Queue Table
-- ============================================================================
...
DECLARE
    ...
    v_total_battle_time INTEGER := 180; -- 3 minute battles
...
BEGIN
    ...
    IF v_battle.started_at IS NOT NULL THEN
        v_elapsed_seconds := EXTRACT(EPOCH FROM (now() - v_battle.started_at))::INTEGER;
        v_remaining_seconds := v_total_battle_time - v_elapsed_seconds;
        IF v_remaining_seconds < 0 THEN
            v_remaining_seconds := 0;
        END IF;
    ELSE
        v_remaining_seconds := v_total_battle_time;
    END IF;
