...
import { ensureUserProfile } from "@/api/supabaseHelpers";
  ...
  useEffect(() => {
    if (user?.id) {
      ensureUserProfile().then(profile => {
        if (profile) {
          console.log('User profile ensured:', profile.username);
        }
      }).catch(error => {
    ...
    queryFn: async () => {
      try {
        ...
        const { data: allLiveStreams = [], error } = await supabase
          .from('streams')
          .select('id,title,thumbnail,category,viewer_count,streamer_id,streamer_name,streamer_avatar,last_heartbeat,created_at,is_live')
          .eq('is_live', true)
    ...
    <main className="min-h-screen bg-gradient-to-br from-[#0a0a0f] via-[#1a0a1f] to-[#0a0a0f] py-10 px-4 relative">
      ...
      <section className="w-full max-w-6xl mx-auto mb-10">
        <div className="relative rounded-xl overflow-hidden bg-black/50">
          <div className="w-full h-72 bg-gradient-to-br from-purple-900/80 to-pink-900/80 flex items-center justify-center">
            <div className="text-center text-white p-8">
              <Crown className="w-16 h-16 mx-auto mb-4 text-yellow-400" />
              <h2 className="text-3xl sm:text-4xl font-extrabold mb-2">
                <AdminContent pageName="Home" fieldName="hero_title" defaultText="MAI Introduces Troll City" />
              </h2>
              <p className="text-gray-300 mb-4">
                <AdminContent pageName="Home" fieldName="hero_subtitle" defaultText="A new line of reinventions. Join the live experience, send gifts, and be a part of the chaos." />
              </p>
