-- ============================================================================
-- FIX: Broadcaster Gift Balance and Notification Issues
-- ============================================================================
-- This migration fixes two issues:
-- 1. Broadcaster balance not updating when gift is sent in broadcast
-- 2. Broadcaster not receiving notification when gift is sent
--
-- The client-side fix is in useGiftSystem.ts which now calls notifyGiftReceived
-- This file updates the database function to also update stream totals
-- ============================================================================

-- Update the send_gift_in_stream function to also update stream's total_gifts_coins
...
BEGIN
  ...
  SELECT cost, name INTO v_gift_cost, v_gift_name
  FROM public.gifts
  WHERE id::text = p_gift_id OR id = p_gift_id OR slug = p_gift_id;
...
SELECT '✅ Migration applied: Broadcaster gift balance and notification fix';
