...
BEGIN
    ...
    LOOP
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE WARNING 'Failed to update function %: %', v_function_name, SQLERRM;
        END;
    END LOOP;
END $$;
...
-- Fix calculate_level - simplified without EXECUTE (always safe)
-- This function will always work regardless of whether profiles table exists
CREATE OR REPLACE FUNCTION public.calculate_level(p_user_id UUID)
RETURNS INTEGER
LANGUAGE plpgsql
STABLE
SET search_path = ''
...
DECLARE
    v_xp INTEGER;
BEGIN
    -- Try to get XP from profiles table, default to 0 if not found
    SELECT COALESCE(total_xp, 0) INTO v_xp 
    FROM public.profiles 
    WHERE id = p_user_id
    LIMIT 1;
    
    -- If no profile found, assume 0 XP
...
-- Fix is_admin (always safe - uses superuser check)
...
BEGIN
    ...
    RETURN current_setting('app.settings.is_admin', true) = 'true' 
           OR current_user = 'postgres'
           OR current_user = 'supabase_admin';
END;
$$;

-- Fix is_staff (always safe)
...
AS $$
BEGIN
    RETURN current_setting('app.settings.is_staff', true) = 'true';
END;
$$;

-- Fix is_staff_on_duty (always safe)
...
AS $$
BEGIN
    RETURN current_setting('app.settings.is_staff_on_duty', true) = 'true';
END;
$$;

-- Fix user_has_vip (always safe)
CREATE OR REPLACE FUNCTION public.user_has_vip(p_user_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
STABLE
SET search_path = ''
...
-- Fix is_not_suspended (always safe)
CREATE OR REPLACE FUNCTION public.is_not_suspended(p_user_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
STABLE
SET search_path = ''
...
-- Fix is_not_banned (always safe)
CREATE OR REPLACE FUNCTION public.is_not_banned(p_user_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
STABLE
SET search_path = ''
...
-- Fix is_user_jailed (always safe)
